...
import os
from configuration import Configuration  # Keep the original import
...
def tavily_search(query, include_raw_content=True, max_results=3):
    ...
    try:
        ...
        if not TAVILY_API_KEY:
            # Fallback for testing purposes only, in production always use environment variables
...
def generate_query(state: SummaryState):
    ...
    try:
        ...
        # Fallback to a simple query using the research topic
...
def summarize_sources(state: SummaryState):
    ...
    try:
        ...
        # Return a minimal valid summary to prevent downstream errors
...
def reflect_on_summary(state: SummaryState):
    try:
        ...
        # Fallback to a simple follow-up query
...
def route_research(state: SummaryState, config: RunnableConfig) -> Literal["finalize_summary", "web_research"]:
    try:
        ...
        # Default to 3 loops if configuration fails
        max_loops = 3
        
    # Improved conditional logic - using < instead of <= to match the intended behavior
